...
export default function handler(req, res) {
  ...
  if (url.includes('/products')) {
    // Check if single product requested
    const match = url.match(/\/products\/([^\/\?]+)/);
    if (match && match[1]) {
      const prod = DEFAULT_PRODUCTS.find(p => p.id === match[1]);
      if (prod) return res.status(200).json(prod);
      return res.status(200).json(DEFAULT_PRODUCTS[0]);
    }
  ...
  if (url.includes('/payments/create-khqr')) {
    return res.status(200).json({
      success: true,
      qrString: '00020101021229300016bakong@abaa0001010210005204599953038405802KH5912Shoply Store6010Phnom Penh62190815ORD-TESTPAY99996304ABCD',
      md5: 'mock-md5-' + Date.now(),
      orderNumber: req.body?.orderNumber || 'ORD-999999'
    });
  }
  ...
  return res.status(200).json({ status: 'ok', provider: 'Vercel Serverless' });
}
